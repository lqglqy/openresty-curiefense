 ...
 * # Safety
 *
 * Returns the block reson. Can be freed with curiefense_str_free.
 */
char *curiefense_cfr_block_reason(struct CFResult *ptr, uintptr_t *ln, uint8_t *is_blocking);

/**
* # Safety
* Returns the CONFIG log string, Can be freed with curiefense_str_free.
*/
char *curiefense_stream_config_log(const struct CFStreamConfig *conf);

#define CF_ACTION_MONITOR 1
#define CF_ACTION_BLOCK 2
#define CF_ACTION_SKIP 3
/**
 * # Safety
 *
 * Returns the log string, json encoded. Can be freed with curiefense_str_free.
 */
char *curiefense_cfr_log_dosec(struct CFResult *ptr, uintptr_t *ln, uint8_t *action,
                              uint32_t *rule_id, uint8_t *risk_level, char *category);
 ...
 * is no longer valid.
 */
enum CFProgress curiefense_async_step_timeout(struct CFExec *ptr, struct CFResult **out, uint64_t max_ms);
 ...
 * data: data for the callback
 */
enum CFProgress curiefense_stream_exec_timeout(const struct CFStreamConfig *config,
                                      struct CFStreamHandle *sh,
                                      void (*cb)(uint64_t),
                                      uint64_t data,
                                      uint64_t max_ms,
...
enum CFProgress curiefense_stream_exec_sync(const struct CFStreamConfig *config,
                                      struct CFStreamHandle *sh,
                                      struct CFResult **out);