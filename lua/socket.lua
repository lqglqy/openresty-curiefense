...
local function _prepare_stream_buffer(socket_obj)
    if socket_obj.sock_type == "tcp" then
        local packet = concat(socket_obj.log_buffer_data, "", 1, socket_obj.log_buffer_index)
        socket_obj.send_tcp_buffer = socket_obj.send_tcp_buffer .. packet
        socket_obj.send_buffer_size = #socket_obj.send_tcp_buffer
    else
        for k,v in ipairs(socket_obj.log_buffer_data) do
            if k > socket_obj.log_buffer_index then
                break
            end
            socket_obj.send_udp_buffer[#socket_obj.send_udp_buffer+1] = v
            socket_obj.send_buffer_size = socket_obj.send_buffer_size + #v
        end
    end
...
local function _do_flush(socket_obj)
    local ok, err, sock, bytes,total_bytes
    local packet_list = {}
    if socket_obj.sock_type == "tcp" then
        packet_list[#packet_list+1] = socket_obj.send_tcp_buffer
    else
        packet_list = socket_obj.send_udp_buffer
    end
    ...
    for k,packet in ipairs(packet_list) do
        bytes, err = sock:send(packet)
        if not bytes then
            -- "sock:send" always closes current connection on error
            ngx_log(ngx.ERR,ngx.now(),":send udp failed with err:"..err)
        else
            if debug then
                ngx.update_time()
...
local function _flush(premature,socket_obj)
    ...
    socket_obj.buffer_size = socket_obj.buffer_size - socket_obj.send_buffer_size
    socket_obj.send_udp_buffer = {}
    socket_obj.send_tcp_buffer = ""
    socket_obj.send_buffer_size = 0
...
function _M:init(user_config)
    ...
    socket_instance.send_udp_buffer = {}
    socket_instance.send_tcp_buffer = ""
    socket_instance.send_buffer_size = 0
